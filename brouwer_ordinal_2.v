...
(** つまり、ブラウワー順序数は、基本列を元にして構築した順序数であると考えることができます。基本列を含んでいるということは、ブラウワー順序数を使用した急増加関数を定義できるということです。 *)
...
(** 旧来のブラウワー順序数は計算可能な形で基本列を有する順序数を取り扱うことができる手法ではありますが、可算順序数という制限があるため、順序数崩壊関数を実装することが困難です。 *)

(** また、構築子の形に一貫性がなく、このままではそこに隠れている本質を読み取ることが困難であり、どのように拡張すればいいのかという道筋が見えづらくなっています。 *)

End 旧来のブラウワー順序数 .

(** ** 複数の段階を有するブラウワー順序数 *)

(** 旧来のブラウワー順序数の定義を複数の段階へ分割することが可能です。この分割とは、あるブラウワー順序数を、それよりも小さいブラウワー順序数を添字とした列の強上限を取る操作について閉じているような型として捉えることです。 *)

(** このように分割することで、ブラウワー順序数の本質を浮かび上がらせ、どのようにすれば非可算順序数を表現できるのか明確になります。このことにより、順序数崩壊関数を実装する方法についても明確になります。 *)

Module 複数の段階を有するブラウワー順序数 .

(** *** ゼロ番目のブラウワー順序数 *)

(** 最初の段階として、ゼロ番目のブラウワー順序数について記述します。 *)

Module ゼロ番目のブラウワー順序数 .

(** ゼロ番目のブラウワー順序数を定義します。 *)

(** ゼロ番目のブラウワー順序数とは、その構築子が存在しないような型です。これは、それよりも小さいブラウワー順序数が存在しないためです。 *)

Inductive ゼロ番目のブラウワー順序数@{ i | } : Type@{ i } := .

(** ゼロ番目のブラウワー順序数の場合分けを定義します。 *)
...
Definition 正規関数を作成する@{ i | } : ゼロ番目のブラウワー順序数@{ i } -> ゼロ番目のブラウワー順序数@{ i }
  := 場合分け@{ i } ゼロ番目のブラウワー順序数@{ i }
.

(** ゼロ番目のブラウワー順序数は空型と同型であり、 << 0 >> と対応します。 *)

End ゼロ番目のブラウワー順序数 .

(** ゼロ番目のブラウワー順序数の省略表記を定義します。 *)
...
(** 次の段階として、一番目のブラウワー順序数について記述します。 *)

Module 一番目のブラウワー順序数 .

(** 一番目のブラウワー順序数を定義します。 *)

(** 一番目のブラウワー順序数とは、ゼロ番目のブラウワー順序数を添字とする列の強上限を取る操作について閉じているような型です。 *)

Inductive 一番目のブラウワー順序数@{ i | } : Type@{ i }
  := ゼロ番目の構築子 : ( ゼロ番目のブラウワー順序数@{ i } -> 一番目のブラウワー順序数 ) -> 一番目のブラウワー順序数
.

(** 一番目のブラウワー順序数の場合分けを定義します。 *)
    ...
    fix a ( 対象 : 一番目のブラウワー順序数@{ i } ) { struct 対象 } : 目標
      :=
        ...
        with
          ゼロ番目の構築子 対象_列
          =>
          ゼロ番目の構築子の処理 ( fun 添字 : ゼロ番目のブラウワー順序数@{ i } => a ( 対象_列 添字 ) )
        end
.

(** 簡易版の一番目のブラウワー順序数の場合分けを定義します。 *)
...
Definition 簡易版の場合分け@{ i | }
  ...
  :=
    ...
    場合分け@{ i }
      目標
      ( fun 対象_列 : ゼロ番目のブラウワー順序数@{ i } -> 目標 => ゼロ番目の構築子の処理 )
.

(** 正規関数を作成する関数を定義します。 *)

Definition 正規関数を作成する@{ i | }
  : 一番目のブラウワー順序数@{ i } -> 一番目のブラウワー順序数@{ i } -> 一番目のブラウワー順序数@{ i }
  := 簡易版の場合分け@{ i } 一番目のブラウワー順序数@{ i }
.

(** ゼロ番目のブラウワー順序数から一番目のブラウワー順序数への埋め込み関数を定義します。 *)
...
(** << ゼロ番目のブラウワー順序数 -> 一番目のブラウワー順序数 >> が単一型と同型であるため、一番目のブラウワー順序数は単一型と同型であり、 << 1 >> と対応します。 *)

End 一番目のブラウワー順序数 .

(** 一番目のブラウワー順序数の省略表記を定義します。 *)
...
(** 次の段階として、二番目のブラウワー順序数について記述します。 *)

Module 二番目のブラウワー順序数 .

(** 二番目のブラウワー順序数を定義します。 *)

(** 二番目のブラウワー順序数とは、ゼロ番目のブラウワー順序数を添字とする列の強上限を取る操作と、一番目のブラウワー順序数を添字とする列の強上限を取る操作について閉じているような型です。 *)
    ...
    fix a ( 対象 : 二番目のブラウワー順序数@{ i } ) { struct 対象 } : 目標
      :=
        ...
        with
          ...
          一番目の構築子 対象_列
          =>
          一番目の構築子の処理 ( fun 添字 : 一番目のブラウワー順序数@{ i } => a ( 対象_列 添字 ) )
        end
.

(** 簡易版の二番目のブラウワー順序数の場合分けを定義します。 *)
...
Definition 簡易版の場合分け@{ i | }
  ...
  :=
    ...
    場合分け@{ i }
      ...
      ( fun 対象_列 : ゼロ番目のブラウワー順序数@{ i } -> 目標 => ゼロ番目の構築子の処理 )
      (
        fun 対象_列 : 一番目のブラウワー順序数@{ i } -> 目標 =>
        一番目の構築子の処理 ( 対象_列 一番目のブラウワー順序数.ゼロ@{ i } )
      )
.
...
Definition 正規関数を作成する@{ i | }
  :
    二番目のブラウワー順序数@{ i }
    ->
    ( 二番目のブラウワー順序数@{ i } -> 二番目のブラウワー順序数@{ i } )
    ->
    二番目のブラウワー順序数@{ i }
    ->
    二番目のブラウワー順序数@{ i }
  := 簡易版の場合分け@{ i } 二番目のブラウワー順序数@{ i }
.

(** ゼロ番目のブラウワー順序数から二番目のブラウワー順序数への埋め込み関数を定義します。 *)
...
(** 一番目のブラウワー順序数から二番目のブラウワー順序数への埋め込み関数を定義します。 *)
...
Definition 加算@{ i | }
  ...
  := fun x : 二番目のブラウワー順序数@{ i } => 正規関数を作成する@{ i } x 後者関数@{ i }
.

(** 乗算を定義します。 *)

(** 順序数の世界では、上付きハイパー演算子の定義ではなく、下付きハイパー演算子の定義を使う必要があります。 *)
...
(** << ゼロ番目のブラウワー順序数 -> 二番目のブラウワー順序数 >> が単一型と同型であり、 << 一番目のブラウワー順序数 -> 二番目のブラウワー順序数 >> が二番目のブラウワー順序数と同型であるため、二番目のブラウワー順序数は自然数の型と同型であり、 << ω >> と対応します。 *)

End 二番目のブラウワー順序数 .

(** 二番目のブラウワー順序数の省略表記を定義します。 *)
...
(** 次の段階として、三番目のブラウワー順序数について記述します。 *)

Module 三番目のブラウワー順序数 .

(** 三番目のブラウワー順序数とは、ゼロ番目のブラウワー順序数を添字とする列の強上限を取る操作と、一番目のブラウワー順序数を添字とする列の強上限を取る操作と、二番目のブラウワー順序数を添字とする列の強上限を取る操作について閉じているような型です。 *)
    ...
    fix a ( 対象 : 三番目のブラウワー順序数@{ i } ) { struct 対象 } : 目標
      :=
        ...
        with
          ...
          二番目の構築子 対象_列
          =>
          二番目の構築子の処理 ( fun 添字 : 二番目のブラウワー順序数@{ i } => a ( 対象_列 添字 ) )
        end
.

(** 簡易版の三番目のブラウワー順序数の場合分けを定義します。 *)
...
Definition 正規関数を作成する@{ i | }
  :
    ...
    三番目のブラウワー順序数@{ i }
  :=
    fun ゼロ番目の構築子の処理 : 三番目のブラウワー順序数@{ i } =>
    fun 一番目の構築子の処理 : 三番目のブラウワー順序数@{ i } -> 三番目のブラウワー順序数@{ i } =>
    簡易版の場合分け@{ i }
      三番目のブラウワー順序数@{ i }
      ゼロ番目の構築子の処理
      一番目の構築子の処理
...
(** ゼロ番目のブラウワー順序数から三番目のブラウワー順序数への埋め込み関数を定義します。 *)
...
(** 一番目のブラウワー順序数から三番目のブラウワー順序数への埋め込み関数を定義します。 *)
...
Definition 加算@{ i | }
  ...
  := fun x : 三番目のブラウワー順序数@{ i } => 正規関数を作成する@{ i } x 後者関数@{ i }
.
...
(** 二番目のブラウワー順序数から三番目のブラウワー順序数への埋め込み関数を定義します。 *)
...
(** 或る値へ或る関数を繰り返し適用した強上限を求める関数を定義します。 *)
...
Definition ヴェブレン階層@{ i | }
  ...
  :=
    簡易版の場合分け@{ i }
      ...
      (
        fun a_列 : 二番目のブラウワー順序数@{ i } -> 三番目のブラウワー順序数@{ i } -> 三番目のブラウワー順序数 =>
        正規関数を作成する@{ i }
          ( 二番目の構築子@{ i } ( fun 添字 : 二番目のブラウワー順序数@{ i } => a_列 添字 ゼロ@{ i } ) )
          (
            fun b_前者 : 三番目のブラウワー順序数@{ i } =>
            二番目の構築子@{ i }
              ( fun 添字 : 二番目のブラウワー順序数@{ i } => a_列 添字 ( 後者関数@{ i } b_前者 ) )
          )
      )
.

(** 三番目のブラウワー順序数は << ω _ 1 >> と対応します。 *)
