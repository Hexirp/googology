...
(** 旧来のブラウワー順序数の定義を複数の段階へ分割することが可能であり、その階層を可算順序数よりも巨大な順序数が表現できるように拡張することができます。 *)
...
(** ゼロ番目のブラウワー順序数を定義します。ゼロ番目のブラウワー順序数とは、その構築子が存在しないような型です。 *)
...
(** ゼロ番目のブラウワー順序数は、空の型と同型であり、 << 0 >> と対応します。 *)
...
(** 一番目のブラウワー順序数とは、ゼロ番目のブラウワー順序数を添字とする列について閉じているような型です。 *)
